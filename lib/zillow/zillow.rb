...
class ZillowScraper
    attr_accessor :location, :pages
    
    def initialize(location='Austin, TX', pages=1)
        @location = location
        @pages = pages
        @current_page = 0
        @boundaries = self.get_location_boundaries(location)
    end

    def get_params
        {
            'searchQueryState'=> "{'pagination':{'currentPage':#{@current_page+1}},'usersSearchTerm':'#{@location}','mapBounds':{'west':#{@boundaries[:west]},'east':#{@boundaries[:east]},'south':#{@boundaries[:south]},'north':#{@boundaries[:north]}},'isMapVisible':true,'filterState':{'sortSelection':{'value':'globalrelevanceex'},'isAllHomes':{'value':true}},'isListVisible':true}",
    ...
    def get_data
        json_object = JSON.parse(get_response.body)
    ...
    def get_hotels
        ...
        @pages.times do |page|
            @current_page = page
            page_results = get_data

            break if page_results.empty?

            data.concat(page_results)
        end
    ...
    def save(data)
        File.open("#{@location.gsub(', ', '_').downcase}.json", 'w', encoding: 'utf-8') do |json_file|
